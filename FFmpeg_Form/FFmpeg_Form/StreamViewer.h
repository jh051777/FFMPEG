	...
	protected:
		...
		void Receive_stream() {
			...
			if (avformat_find_stream_info(fmt_ctx, NULL) < 0) {
				ShowError("Failed to receive stream info.");
			...
			while (isStreaming) {
				...
				if (frame_q.size() > 20) {
					...
					catch (Exception^ ex)
					{
						...
						Console::WriteLine("Unknown exception caught during frame processing.");
						av_frame_free(&frame);
						continue;
					}*/
					av_frame_free(&frame);
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->lbl_time->Location = System::Drawing::Point(276, 732);
